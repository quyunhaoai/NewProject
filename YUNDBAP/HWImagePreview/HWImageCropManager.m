...
@implementation HWImageCropManager
...
+ (UIImage *)cropImageView:(UIImageView *)imageView toRect:(CGRect)rect zoomScale:(double)zoomScale containerView:(UIView *)containerView {
    ...
    CGFloat xMargin = containerView.HW_width - CGRectGetMaxX(rect) - rect.origin.x;
    ...
    cropedImage = [[HWImageManager manager] fixOrientation:cropedImage];
...
@implementation UIImage (HWGif)

+ (UIImage *)sd_HW_animatedGIFWithData:(NSData *)data {
    if (!data) {
        return nil;
    }

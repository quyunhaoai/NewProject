...
@implementation HWAssetCell

- (void)setModel:(HWAssetModel *)model {
    _model = model;
    if (iOS8Later) {
        self.representedAssetIdentifier = [[HWImageManager manager] getAssetIdentifier:model.asset];
    }
    int32_t imageRequestID = [[HWImageManager manager] getPhotoWithAsset:model.asset photoWidth:self.HW_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (_progressView) {
        ...
        if ([self.representedAssetIdentifier isEqualToString:[[HWImageManager manager] getAssetIdentifier:model.asset]]) {
...
- (void)setType:(HWAssetCellType)type {
    _type = type;
    if (type == HWAssetCellTypePhoto || type == HWAssetCellTypeLivePhoto || (type == HWAssetCellTypePhotoGif && !self.allowPickingGif) || self.allowPickingMultipleVideo) {
    ...
    if (type == HWAssetCellTypeVideo) {
        self.bottomView.hidden = NO;
        self.timeLength.text = _model.timeLength;
        self.videoImgView.hidden = NO;
        _timeLength.HW_left = self.videoImgView.HW_right;
        _timeLength.textAlignment = NSTextAlignmentRight;
    } else if (type == HWAssetCellTypePhotoGif) {
        self.bottomView.hidden = NO;
        self.timeLength.text = @"GIF";
        self.videoImgView.hidden = YES;
        _timeLength.HW_left = 5;
...
- (void)selectPhotoButtonClick:(UIButton *)sender {
    ...
    if (sender.isSelected) {
        [UIView showOscillatoryAnimationWithLayer:_selectImageView.layer type:HWOscillatoryAnimationToBigger];
        // 用户选中了该图片，提前获取一下大图
        [self fetchBigImage];
    } else { // 取消选中，取消大图的获取
...
- (void)fetchBigImage {
    _bigImageRequestID = [[HWImageManager manager] getPhotoWithAsset:_model.asset completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
        if (_progressView) {
            [self hideProgressView];
        }
...
- (void)layoutSubviews {
    ...
    _selectPhotoButton.frame = CGRectMake(self.HW_width - 44, 0, 44, 44);
    _selectImageView.frame = CGRectMake(self.HW_width - 27, 0, 27, 27);
    _imageView.frame = CGRectMake(0, 0, self.HW_width, self.HW_height);
    
    static CGFloat progressWH = 20;
    CGFloat progressXY = (self.HW_width - progressWH) / 2;
    _progressView.frame = CGRectMake(progressXY, progressXY, progressWH, progressWH);

    _bottomView.frame = CGRectMake(0, self.HW_height - 17, self.HW_width, 17);
    _videoImgView.frame = CGRectMake(8, 0, 17, 17);
    _timeLength.frame = CGRectMake(self.videoImgView.HW_right, 0, self.HW_width - self.videoImgView.HW_right - 5, 17);
...
- (void)setModel:(HWAlbumModel *)model {
    _model = model;
    ...
    [[HWImageManager manager] getPostImageWithAlbumModel:model completion:^(UIImage *postImage) {
...
- (void)layoutSubviews {
    ...
    _selectedCountButton.frame = CGRectMake(self.HW_width - 24 - 30, 23, 24, 24);
}
...
- (UILabel *)titleLabel {
    if (_titleLabel == nil) {
        ...
        titleLabel.frame = CGRectMake(80, 0, self.HW_width - 80 - 50, self.HW_height);
...
- (UIImageView *)arrowImageView {
    if (_arrowImageView == nil) {
        ...
        CGFloat arrowWH = 15;
        arrowImageView.frame = CGRectMake(self.HW_width - arrowWH - 12, 28, arrowWH, arrowWH);
...
@implementation HWAssetCameraCell

...
- (UIImageView *)arrowImage
{
    if (!_arrowImage) {
        ...
        UIImage *image_arrow = nil;
        if ([self.ssBase.refresImageType isEqualToString:@"0"]) {
            image_arrow = [UIImage imageNamed:@"icon_default_arrow"];
        }else {
            image_arrow = [UIImage imageNamed:@"icon_arrow"];
        }
        
        UIImageView *arrowImage = [[UIImageView alloc] initWithImage:image_arrow];
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _circleView = [[CircleView alloc] initWithFrame:CGRectMake(85, 10, 26, 26)];
        _circleView.bounds = CGRectMake(0, 0, 30, 30);
        _circleView.autoresizingMask = self.arrowImage.autoresizingMask;
        if (![self.ssBase.refresImageType isEqualToString:@"0"]) {
...
- (void)layoutSubviews
{
    ...
    if ([self.ssBase.refresImageType isEqualToString:@"0"]) {
        self.arrowImage.bounds = CGRectMake(0, 0, 26, 17);
    }else {
        self.arrowImage.bounds = CGRectMake(0, 0, 11, 10);
    }
    // 2.指示器
    self.activityView.center = self.arrowImage.center;
    _circleView.center = self.arrowImage.center;
}
...
#pragma mark -不能调用set方法-

            _state = MJRefreshStateWillRefreshing;
            
#pragma mark -为了保证在viewWillAppear等方法中也能刷新-

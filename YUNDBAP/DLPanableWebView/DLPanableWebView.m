...
- (UIImageView *)webleftImgView{
    if (!_webleftImgView) {
        
        _webleftImgView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"webleft"]];
        [[UIApplication sharedApplication].keyWindow addSubview:_webleftImgView];
        
    }
    
    return _webleftImgView;
}
- (void)commonInit{
    ...
    [self changeLeftImg:-64];
    
}

- (void)changeLeftImg:(float)leftX {
    if (leftX > 0) {
        leftX -= 64;
        if (leftX > 0) {
            leftX = 0;
    ...
    //    NSLog(@"left坐标…%f",leftX);
    [self webleftImgView].frame = CGRectMake(leftX, [UIScreen mainScreen].bounds.size.height/2, 52, 64);
}
